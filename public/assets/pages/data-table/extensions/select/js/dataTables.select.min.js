...
e.each(c.aanFeatures.i,function(c,b){var b=e(b),d=b.children("span.select-info");d.length&&d.remove();""!==a.text()&&b.append(a)})}}function z(b,c,a,d){var f=b[c+"s"]({Buscar:"applied"}).indexes(),d=e.inArray(d,f),m=e.inArray(a,f);if(!b[c+"s"]({selected:!0}).any()&&-1===d)f.splice(e.inArray(a,f)+1,f.length);else{if(d>m)var k=m,m=d,d=k;f.splice(m+1,f.length);f.splice(0,d)}b[c](a,{selected:!0}).any()?(f.splice(e.inArray(a,f),1),b[c+"s"](f).deselect()):b[c+"s"](f).select()}function p(b,c){if(c||"single"===
